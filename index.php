...
<head>
    ...
    <style>
        ...
        .floating-social-icons {
            position: fixed;
            /* Key: Makes it fixed when scrolling */
            top: 50%;
            right: 0;
        ...
        .floating-social-icons a {
            ...
            /* Rounded on the left, flat on the right */
        ...
        @media (max-width: 767px) {
            .floating-social-icons {
                top: 70%;
                /* ⬇️ Key: 50% থেকে 70% এ নামানো হয়েছে */
                opacity: 0.5;
                /* ⬇️ Key: 50% opacity as requested for mobile */
            }
...
<body class="bg-gray-50 font-sans">
    ...
    <div class="relative w-full h-[500px] sm:h-[400px] md:h-[500px] lg:h-[500px] bg-gray-100 shadow-xl">
        ...
        <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-20">
            <?php foreach ($banners as $i => $b): ?>
                <button class="slide-indicator w-3 h-3 rounded-full transition duration-300 <?= $i === 0 ? 'bg-white' : 'bg-white/50 hover:bg-white/80' ?>" data-slide-index="<?= $i ?>"></button>
            <?php endforeach; ?>
        </div>
...
<script>
    ...
    if (totalSlides > 0) {
        ...
        function showSlide(index) {
            // Normalize index (looping slider)
            ...
            indicators[index].classList.remove('bg-white/50', 'hover:bg-white/80');
        ...
        // Initial setup (ensuring the first slide/dot is correct)
 ...
 <script>
    // Photo Gallery Slider Script (Renamed variables to prevent conflict)
    let photoCurrentSlide = 0;
    const photoSliderTrack = document.getElementById('slider-track');
    const photoSlides = photoSliderTrack.children;
    const photoTotalSlides = photoSlides.length;

    function showPhotoSlide(index) {
        if (index < 0) index = photoTotalSlides - 1;
        if (index >= photoTotalSlides) index = 0;
        photoCurrentSlide = index;
        photoSliderTrack.style.transform = `translateX(-${photoCurrentSlide * 100}%)`;
    }

    function nextPhotoSlide() {
        showPhotoSlide(photoCurrentSlide + 1);
    }

    // Auto-slide every 5 seconds
    if (photoTotalSlides > 1) { // Only auto-slide if there's more than one slide
        setInterval(() => {
            nextPhotoSlide();
        }, 5000);
    }
